...
function getVisibleZodiacColor(hexString) {
    ...
    if (hsl.l > 0.55) {
        color.setHSL(hsl.h, Math.min(1.0, hsl.s + 0.2), Math.max(0.12, hsl.l - 0.45));
    } else {
        color.setHSL(hsl.h, Math.min(1.0, hsl.s + 0.2), Math.min(0.92, hsl.l + 0.45));
    }
...
function createColorizedZodiacTexture(sign, hexColor, callback) {
    ...
    if (zodiacCanvasTextureCache[cacheKey]) {
        callback(zodiacCanvasTextureCache[cacheKey]);
        return;
    }
...
function createEntity(x, y, type, tipType, zodiac, bladeColor1, bladeColor2, coreColor, name, isPlayer = false) {
    return {
        x: x, y: y,
        vx: 0, vz: 0,
        radius: 28,
        ...
        cooldownTimer: 0, isCooldown: false,
        prevX: x, prevY: y,
...
function startGameSession(playerList, mode) {
    ...
    if (dashBtn) {
        ...
        dashBtn.style.borderColor = getVisibleZodiacColor(cCol);
    ...
    if (timerRing) {
        timerRing.style.strokeDashoffset = 264;
    }
    ...
    entityTrails.forEach(t => { if (t && t.mesh) beybladeScene.remove(t.mesh); });
    entityTrails = [];
...
function returnToLobby() {
    ...
    entityMeshes.forEach(bm => { if (bm) beybladeScene.remove(bm); });
    entityMeshes = [];
    entityTrails.forEach(t => { if (t && t.mesh) beybladeScene.remove(t.mesh); });
    entityTrails = [];
    activeSparks.forEach(s => { if (s && s.mesh) beybladeScene.remove(s.mesh); });
...
const showroomCam = new THREE.PerspectiveCamera(40, 1, 0.1, 1000);
showroomCam.position.set(0, 60, 95);
showroomCam.lookAt(0, 0, 0);

const showroomRenderer = new THREE.WebGLRenderer({ antialias: true });
showroomRenderer.setSize(180, 180);
...
const sLight = new THREE.DirectionalLight(0xffffff, 0.75);
sLight.position.set(50, 150, 50);
showroomScene.add(sLight);

let showroomAngleY = 0, showroomVelY = 0;
let showroomAngleX = 0, showroomVelX = 0;
let isShowroomDragging = false;
let prevMouseX = 0, prevMouseY = 0;

const showroomFrame = document.getElementById('showroom-frame');
if (showroomFrame) {
    showroomFrame.addEventListener('pointerdown', (e) => {
        isShowroomDragging = true;
    ...
    window.addEventListener('pointermove', (e) => {
        ...
        showroomVelY = dx * 0.03;
        showroomVelX = dy * 0.03;
        showroomAngleY += showroomVelY;
        showroomAngleX += showroomVelX;
        prevMouseX = e.clientX;
        prevMouseY = e.clientY;
    });
    window.addEventListener('pointerup', () => {
        isShowroomDragging = false;
    });
}
...
function createBeybladeMesh(bladeType, tipType, zodiacType, bladeCol1, bladeCol2, coreCol) {
    ...
    containerGroup.add(spinGroup);
    containerGroup.add(staticCoreGroup);
    
    let statInfo = PARTS_STAT[bladeType] || PARTS_STAT['circle'];
    let glbFileName = statInfo.glb || 'custom_balance.glb';
    let glbPath = `./assets/models/${glbFileName}`;
    ...
    gltfLoader.load(glbPath, (gltf) => {
        ...
        model.traverse((child) => {
            if (child.isMesh && child.material) {
                ...
                if (mName.includes('main') || mName.includes('1') || mName.includes('body')) {
                    child.material.color.set(bladeCol1);
                } else if (mName.includes('accent') || mName.includes('2') || mName.includes('point') || mName.includes('sub')) {
                    child.material.color.set(bladeCol2);
                } else {
                    child.material.color.set(bladeCol1);
                }
    ...
    let tipGlbFileName = tipStat.glb || 'custom_tip_speed.glb';
    let tipGlbPath = `./assets/models/${tipGlbFileName}`;

    gltfLoader.load(tipGlbPath, (gltf) => {
        const tipModel = gltf.scene;
        tipModel.scale.set(50, 50, 50);
        tipModel.position.y = 5; 
        ...
        tipModel.traverse((child) => {
            if (child.isMesh && child.material) {
                ...
                if (tipType === 'heavy') {
                    child.material.color.set(0x475569); 
                } else {
                    child.material.color.set(0x94a3b8); 
                }
            }
        });
    ...
    gltfLoader.load('./assets/models/custom_core.glb', (gltf) => {
        ...
        coreModel.scale.set(50, 50, 50);
        coreModel.position.y = 11.5;
        coreModel.traverse((child) => {
            if (child.isMesh && child.material) {
        ...
        const coreMesh = new THREE.Mesh(coreGeo, jewelMaterial);
        coreMesh.position.y = 11.5;
        staticCoreGroup.add(coreMesh);
    });
    ...
    if (zodiacType) {
        ...
        bitChip.rotation.x = -Math.PI / 2;
        bitChip.position.set(0, 19.8, 0); 
        bitChip.renderOrder = 999;        
        bitChip.name = 'bitChip';
        staticCoreGroup.add(bitChip);
    }
    ...
    for (let i = 0; i < particleCount; i++) {
        ...
        let speed = 0.06 + Math.random() * 0.04;
        let riseSpeed = 0.7 + Math.random() * 0.8;
    ...
    const tornadoParticles = new THREE.Points(pGeo, pMat);
    tornadoParticles.name = 'tornadoParticles';
    tornadoParticles.userData = { particleData };
    spinGroup.add(tornadoParticles);
...
gltfLoader.load('./assets/models/custom_arena.glb', (gltf) => {
        ...
        if (child.isMesh && child.material) {
            ...
            materials.forEach((mat) => {
                ...
                if (mName.includes('logo_decal')) {
                    let fileName = '';
                    if (mName.includes('center')) fileName = 'logo_center.png';
                    else if (mName.includes('topleft')) fileName = 'logo_top_left.png';
                    else if (mName.includes('topright')) fileName = 'logo_top_right.png';
                    else if (mName.includes('bottomleft')) fileName = 'logo_bottom_left.png';
                    else if (mName.includes('bottomright')) fileName = 'logo_bottom_right.png';
                    else if (mName.includes('top')) fileName = 'logo_top.png';
                    else if (mName.includes('bottom')) fileName = 'logo_bottom.png';
                ...
                else if (mName.includes('arena_floor_mat') || mName.includes('floor')) {
                    mat.color.set(0xcccccc); 
                    mat.roughness = 0.3;     
                    mat.metalness = 0.05;
                    mat.needsUpdate = true;
                }
                else if (mName.includes('arena_wall_mat') || mName.includes('wall')) {
    ...
    outerWall.position.y = -22;
    gameScene.add(outerWall);

    const ring = new THREE.Mesh(
        new THREE.TorusGeometry(arenaRadius, 5.5, 16, 100), 
        new THREE.MeshStandardMaterial({ color: '#ef4444', emissive: 0x991b1b, emissiveIntensity: 0.9 })
    );
...
window.addEventListener('pointermove', (e) => {
    ...
    inputDirX = dx / maxDist;
    inputDirZ = dy / maxDist;
});
...
function triggerDash() {
    ...
    let dirX = inputDirX || 0, dirZ = inputDirZ || -1;
    let len = Math.sqrt(dirX * dirX + dirZ * dirZ) || 1;
    
    player.isDashing = true;
    player.hasHit = false; 
    player.hasScoredHit = false; 
    player.dashFrames = 10; 
    player.dashDirX = (dirX / len) * 7; 
...
function createCollisionSpark(x, y) {
    let sparkGeo = new THREE.SphereGeometry(2.5, 6, 6);
    ...
    beybladeScene.add(spark);
    activeSparks.push({ mesh: spark, life: 6, maxLife: 6 });
}

function renderLayeredScene() {
    gameRenderer.clear();
    gameRenderer.render(gameScene, gameCam);
    gameRenderer.clearDepth();
    ...
    try {
        if (!gameStarted) {
            if (document.getElementById('custom-modal').style.display === 'flex' || document.getElementById('multiplayer-modal').style.display === 'flex' || document.getElementById('lobby-screen').style.display !== 'none') {
                if (!isShowroomDragging) {
                    showroomAngleY += (0 - showroomAngleY) * 0.18;
                    showroomAngleX += (0 - showroomAngleX) * 0.18;
                    showroomVelY *= 0.8;
                    showroomVelX *= 0.8;
                }
                if (showroomTop && showroomTop.userData && showroomTop.userData.spinGroup) {
                    showroomTop.userData.spinGroup.rotation.y -= 0.05;
                    showroomTop.rotation.y = showroomAngleY;
                    showroomTop.rotation.x = showroomAngleX;
                }
            ...
            if (gamePhase === 'countdown') {
                ...
                let numElem = document.getElementById('countdown-number');
                if (numElem) {
                    if (countdownTimer > 120) numElem.innerText = '3';
                    else if (countdownTimer > 60) numElem.innerText = '2';
                    else if (countdownTimer > 0) numElem.innerText = '1';
                    else {
                        numElem.innerText = 'GO!';
                ...
                if (player) {
                    gameCam.position.x = player.x;
                    gameCam.position.z = player.y + 220;
                    gameCam.position.y = 320;
                    gameCam.lookAt(player.x, 0, player.y);
                }
                ...
                entities.forEach((ent, i) => {
                    ...
                    if (entityMeshes[i]) {
                        entityMeshes[i].position.set(ent.x, Math.max(0, ent.dropHeight), ent.y);
                        if (entityMeshes[i].userData && entityMeshes[i].userData.spinGroup) {
                            entityMeshes[i].userData.spinGroup.rotation.y -= 0.5;
                        }
                    }
                    if (entityTrails[i]) {
                        entityTrails[i].mesh.visible = false;
                    }
                });
            ...
            if (gamePhase === 'playing') {
                entities.forEach((ent) => {
                    ...
                    if (ent.isStunned) {
                        ent.stunTimer--;
                        if (ent.stunTimer <= 0) ent.isStunned = false;
                        ent.vx *= 0.88; ent.vz *= 0.88;
                    ...
                    if (distToCenter > 5 && !(ent.isDashing && ent.isPlayer)) {
                        ent.vx -= (ent.x / distToCenter) * 0.07;
                        ent.vz -= (ent.y / distToCenter) * 0.07;
                    }
                ...
                entities.forEach((ent, i) => {
                    ...
                    if (dCenter + ent.radius > arenaRadius) {
                        if (gameMode === 'quick') {
                            ...
                            ent.isAlive = false;
                            eliminationOrder.unshift(ent);
                            if (entityMeshes[i]) beybladeScene.remove(entityMeshes[i]);
                            if (entityTrails[i]) {
                                beybladeScene.remove(entityTrails[i].mesh);
                                entityTrails[i] = null;
                            }
                        }
                    }
                ...
                if (gameMode === 'private') {
                    ...
                    if (aliveEntities.length <= 1) {
                        if (aliveEntities.length === 1) {
                            eliminationOrder.unshift(aliveEntities[0]);
                        }
                        gamePhase = 'gameover';
                        showResults();
                    }
            ...
            for (let s = activeSparks.length - 1; s >= 0; s--) {
                let sp = activeSparks[s];
                sp.life--;
                sp.mesh.scale.multiplyScalar(1.08);
                sp.mesh.material.opacity = (sp.life / sp.maxLife) * 0.6;
                if (sp.life <= 0) {
                    beybladeScene.remove(sp.mesh);
            ...
            entities.forEach((ent, i) => {
                ...
                if (bm.userData && bm.userData.spinGroup) {
                    ...
                    if (aura) {
                        if (ent.comboCount > 0 && ent.comboTimer > 0) {
                            let progress = ent.comboTimer / 300;
                            aura.material.opacity = Math.min(0.4 + (ent.comboCount * 0.15), 0.95) * progress;

                            let positions = aura.geometry.attributes.position.array;
                            let data = aura.userData.particleData;
                            for (let j = 0; j < data.length; j++) {
                                data[j].angle += data[j].speed;
                                data[j].y += data[j].riseSpeed;
                                if (data[j].y > 35) {
                                    data[j].y = 0;
                                    data[j].radius = 8 + Math.random() * 10;
                                }
                                let currentRadius = data[j].radius * (0.7 + Math.sin(data[j].y / 35 * Math.PI) * 0.5);
                                positions[j * 3] = Math.cos(data[j].angle) * currentRadius;
                                positions[j * 3 + 1] = data[j].y;
                                positions[j * 3 + 2] = Math.sin(data[j].angle) * currentRadius;
                            }
                ...
                if (trail) {
                    let currentPos = new THREE.Vector3(ent.x, 0.6, ent.y);
                    let speed = Math.sqrt(ent.vx * ent.vx + ent.vz * ent.vz);
                    
                    if (speed > 0.02) {
                        if (trail.points.length === 0) {
                            for (let k = 0; k <= trail.segs; k++) trail.points.push(currentPos.clone());
                        } else {
                            let dist = trail.points[0].distanceTo(currentPos);
                            if (dist > 0.8) {
                                trail.points.unshift(currentPos.clone());
                                if (trail.points.length > trail.segs + 1) {
                                    trail.points.pop();
                                }
                            } else {
                                trail.points[0].lerp(currentPos, 0.5);
                            }
                        }
                    } else {
                    ...
                    let hasMoved = trail.points.length > 2 && speed > 0.02;
                    trail.mesh.visible = hasMoved;

                    if (hasMoved) {
                        let posAttr = trail.mesh.geometry.attributes.position;
                        let uvAttr = trail.mesh.geometry.attributes.uv;
                        let lastPerp = null;

                        for (let j = 0; j <= trail.segs; j++) {
                            let ptIndex = Math.min(j, trail.points.length - 1);
                            let pt = trail.points[ptIndex];
                            
                            let nextPt = trail.points[Math.min(j + 1, trail.points.length - 1)];
                            let dir = new THREE.Vector3().subVectors(pt, nextPt);
                            ...
                            let left = pt.clone().addScaledVector(perp, width * 0.5);
                            let right = pt.clone().addScaledVector(perp, -width * 0.5);

                            posAttr.setXYZ(j * 2, left.x, left.y, left.z);
                            posAttr.setXYZ(j * 2 + 1, right.x, right.y, right.z);

                            let v = j / trail.segs;
                            uvAttr.setXY(j * 2, 0.0, v);
                            uvAttr.setXY(j * 2 + 1, 1.0, v);
                        }
                        ...
                        trail.mesh.geometry.computeVertexNormals();
                    }
                }
            });
            ...
            if (player) {
                ...
                const timerRing = document.getElementById('dash-timer-ring');
                const textContent = document.getElementById('dash-text-content');
                const dashIconMask = document.getElementById('dash-icon-mask');

                if (gameMode === 'private') {
                    if (!player.isAlive && gamePhase === 'playing') {
                        if (ingameChatBox) ingameChatBox.style.display = 'block';

                        if (dashBtn && textContent && timerRing) {
                            dashBtn.className = "sub";
                            dashBtn.style.background = "linear-gradient(135deg, #3b82f6, #1d4ed8)";
                            dashBtn.style.borderColor = "#1e40af";
                            dashBtn.style.boxShadow = "none";
                            if (dashIconMask) dashIconMask.style.display = "none";
                            textContent.style.display = "block";
                            textContent.innerHTML = "👁️"; 
                            timerRing.style.strokeDashoffset = 264;
                        }
                    } else {
                        if (ingameChatBox) ingameChatBox.style.display = 'none';
                ...
                if (player.isAlive) {
                    if (dashBtn && timerRing && textContent) {
                        let baseColor = player.coreColor;
                        let visibleIconColor = getVisibleZodiacColor(baseColor);
                        
                        if (dashIconMask) dashIconMask.style.display = "block";
                        
                        if (player.isCooldown) {
                            dashBtn.className = "cooldown";
                            dashBtn.style.background = "#334155";
                            dashBtn.style.borderColor = "#1e293b";
                            dashBtn.style.boxShadow = "none";
                            if (dashIconMask) dashIconMask.style.backgroundColor = "#94a3b8"; 
                            
                            textContent.style.display = "block";
                            textContent.innerHTML = `${Math.ceil(player.cooldownTimer / 60)}s`;
                            timerRing.style.strokeDashoffset = 264;
                        } else {
                            if (player.comboCount > 0) {
                                dashBtn.className = "glowing";
                                dashBtn.style.background = baseColor;
                                dashBtn.style.borderColor = "#ffffff";
                                dashBtn.style.boxShadow = `0 0 20px ${baseColor}`;
                                if (dashIconMask) dashIconMask.style.backgroundColor = "#ffffff"; 
                                
                                textContent.style.display = "block";
                                textContent.innerHTML = `x${player.comboCount}`;
                                let progressFraction = player.comboTimer / 300;
                                timerRing.style.strokeDashoffset = 264 * (1 - progressFraction);
                            } else {
                                dashBtn.className = "";
                                dashBtn.style.background = baseColor;
                                dashBtn.style.borderColor = visibleIconColor; 
                                dashBtn.style.boxShadow = "none";
                                if (dashIconMask) dashIconMask.style.backgroundColor = visibleIconColor; 
                    ...
                    let targetCamZ = player.y + 240;
                    gameCam.position.x += (targetCamX - gameCam.position.x) * 0.08;
                    gameCam.position.z += (targetCamZ - gameCam.position.z) * 0.08;
                    gameCam.position.y = 400;
                    gameCam.lookAt(player.x, 0, player.y);
                } else if (gameMode === 'private') {
                    let aliveEntities = entities.filter(e => e.isAlive);
                    if (aliveEntities.length > 0) {
                        if (spectateTargetIndex >= aliveEntities.length) {
                            spectateTargetIndex = 0;
                        }
                        let spectateTarget = aliveEntities[spectateTargetIndex];
                        gameCam.position.x += (spectateTarget.x - gameCam.position.x) * 0.08;
                        gameCam.position.z += ((spectateTarget.y + 240) - gameCam.position.z) * 0.08;
                        gameCam.position.y = 420;
                        gameCam.lookAt(spectateTarget.x, 0, spectateTarget.y);
                    } else {
                        gameCam.position.x += (0 - gameCam.position.x) * 0.08;
                        gameCam.position.z += (300 - gameCam.position.z) * 0.08;
                        gameCam.position.y = 500;
                        gameCam.lookAt(0, 0, 0);
                    }
                }
            }
            ...
            if (statusText && gamePhase === 'playing' && player) {
                if (!player.isAlive) {
                    statusText.innerHTML = "💀 SPECTATING - Click VIEW button to switch targets!";
                    statusText.style.color = "#ef4444";
                } else if (player.isStunned) {
                    statusText.innerHTML = "⚠️ OVERHEAT STUN!";
                    statusText.style.color = "#ef4444";
                } else if (player.isDashing) {
                    statusText.innerHTML = "⚡ DASHING!";
                    statusText.style.color = "#f59e0b";
                } else if (player.comboCount > 0) {
                    statusText.innerHTML = `🔥 COMBO x${player.comboCount} - POWER BOOSTED!`;
                    statusText.style.color = "#38bdf8";
                } else {
                    statusText.innerHTML = gameMode === 'quick' ? "Survive as long as you can!" : "Last one standing wins!";
            ...
            if (rankListElem) {
                if (gameMode === 'quick') {
                    ...
                    let rankHTML = '';
                    rankData.forEach(r => {
                        let isP = (player && r.name === player.name);
                        let col = isP ? '#f1c40f' : '#cbd5e1';
...
function showResults() {
    ...
    eliminationOrder.forEach((ent, idx) => {
        ...
        html += `<div style="display:flex; justify-content:space-between; font-size:12px; padding:5px 0; border-bottom:1px solid #334155; font-weight:bold;">
            <span>${badge} - <span style="color:${ent.bladeColor1}">${ent.name}</span></span>
            <span>${idx === 0 ? '🏆 WINNER' : 'Eliminated'}</span>
        </div>`;
    });
