...
window.addBotToPrivateRoom = function() {
    ...
    let botData = {
        id: botId, name: bName, isHost: false, isBot: true,
...
function spawnEntityDynamic(pd) {
    let spawnRadius = 140;
    let angle = Math.random() * Math.PI * 2;
    let sx = Math.cos(angle) * spawnRadius;
    let sy = Math.sin(angle) * spawnRadius;
    
    let ent = createEntity(pd.id, sx, sy, pd.blade || 'circle', pd.tip || 'speed', pd.zodiac || 'rat', pd.color1 || '#ef4444', pd.color2 || '#b91c1c', pd.core || '#f59e0b', pd.name || 'Player', false, false);
    entities.push(ent);
...
function createDynamicTrail(bladeColor) {
    ...
    let mat = new THREE.MeshBasicMaterial({ 
        color: bladeColor, map: getTrailTexture(), 
...
function createEntity(id, x, y, type, tipType, zodiac, bladeColor1, bladeColor2, coreColor, name, isPlayer, isBot) {
    return {
        ...
        targetX: x, targetY: y, targetVx: 0, targetVz: 0
    };
}
...
function setupNetworkSync() {
    let syncRef = (gameMode === 'private') ? roomRef : publicRoomRef;
    if (syncRef) {
        syncRef.child('gameStates').on('value', (snapshot) => {
            ...
            for (let pid in states) {
                if (pid !== mySessionId) {
                    ...
                    if (target && target.isAlive) {
                        ...
                        target.targetVx = states[pid].vx;
                        target.targetVz = states[pid].vz;
...
window.returnToLobby = function() {
    ...
    if (gameMode === 'private' && roomRef) { window.leaveRoom(); }
    if (gameMode === 'quick' && publicRoomRef) {
        publicRoomRef.child('players/' + mySessionId).remove();
        publicRoomRef.child('gameStates/' + mySessionId).remove();
        publicRoomRef.off();
    }
};
...
window.restartCurrentGame = function() {
    if (gameMode === 'private') {
        if (isHost) {
            document.getElementById('result-modal').style.display = 'none';
            window.startPrivateGameSession();
        } else {
            alert('방장이 게임을 다시 시작할 때까지 기다려주세요!');
        }
    } else {
        document.getElementById('result-modal').style.display = 'none';
...
if (showroomFrame) {
    showroomFrame.addEventListener('pointerdown', (e) => { 
        isShowroomDragging = true; prevMouseX = e.clientX; prevMouseY = e.clientY; 
    });
...
function createBeybladeMesh(bladeType, tipType, zodiacType, bladeCol1, bladeCol2, coreCol) {
    ...
    gltfLoader.load(`./assets/models/${tipStat.glb || 'custom_tip_speed.glb'}`, (gltf) => {
        ...
        if (tipType === 'speed') tip.rotation.x = Math.PI; 
        tip.position.y = 5; 
        spinGroup.add(tip);
    });

    const jewelMaterial = new THREE.MeshPhysicalMaterial({ 
        color: coreCol, metalness: 0.1, roughness: 0.15, transmission: 0.2, opacity: 1.0, 
        transparent: true, ior: 1.5, side: THREE.DoubleSide, clearcoat: 1.0, clearcoatRoughness: 0.05 
    });
    ...
    gltfLoader.load('./assets/models/custom_core.glb', (gltf) => {
        ...
        coreModel.traverse((child) => { 
            if (child.isMesh && child.material) { child.material = jewelMaterial; } 
        });
...
window.updateShowroom = function() { 
    ...
    showroomTop = createBeybladeMesh(
        document.getElementById('select-blade').value, 
        document.getElementById('select-tip').value, 
        document.getElementById('select-zodiac').value || 'rat', 
        document.getElementById('blade-color-1').value, 
        document.getElementById('blade-color-2').value, 
        document.getElementById('core-color').value
    ); 
    showroomScene.add(showroomTop); 
};
                ...
                if (mName.includes('logo_decal')) {
                    ...
                    if (fileName) { 
                        textureLoader.load(`./assets/textures/ads/${fileName}`, (tex) => { 
                            ...
                            tex.encoding = THREE.sRGBEncoding; 
                            tex.center.set(0.5, 0.5); 
                            if (mName.includes('center')) { tex.rotation = Math.PI; } else { tex.repeat.x = -1; } 
                            mat.map = tex; mat.transparent = true; mat.alphaTest = 0.1; mat.depthWrite = true; mat.opacity = 1.0; mat.needsUpdate = true; 
                        }, undefined, () => { 
                            mat.transparent = true; mat.opacity = 0.0; mat.needsUpdate = true; 
                        }); 
                    }
                } 
                else if (mName.includes('arena_floor_mat') || mName.includes('floor')) { 
                    mat.color.set(0xcccccc); mat.roughness = 0.3; mat.metalness = 0.05; mat.needsUpdate = true; 
                }
                else if (mName.includes('arena_wall_mat') || mName.includes('wall')) { 
                    mat.color.set(0x475569); mat.roughness = 0.05; mat.metalness = 0.85; mat.needsUpdate = true; 
                }
            });
        }
    });
            ...
            if (gamePhase === 'playing') {
                ...
                entities.forEach((ent) => {
                    ...
                    if (ent.isStunned) {
                        ...
                        if (ent.dashFrames <= 0) { 
                            ent.isDashing = false; 
                            if (!ent.hasHit) { 
                                ent.isStunned = true; ent.stunTimer = 60; 
                                ent.isCooldown = true; ent.cooldownTimer = 300; 
                                ent.comboCount = 0; 
                            } 
                        }
                    } else {
                        if (ent.isPlayer) {
                            ...
                            if (target) {
                                ...
                                if (ent.aiState === 'attack') { 
                                    ent.vx += (dx / d) * 0.35; ent.vz += (dz / d) * 0.35; 
                                } else if (ent.aiState === 'circle') { 
                                    let perpX = -dz / d; let perpZ = dx / d; 
                                    ent.vx += (perpX * 0.3 + (dx / d) * 0.12); ent.vz += (perpZ * 0.3 + (dz / d) * 0.12); 
                                } else if (ent.aiState === 'defend') { 
                                    let toCenterX = -ent.x; let toCenterZ = -ent.y; 
                                    let cDist = Math.sqrt(toCenterX*toCenterX + toCenterZ*toCenterZ) || 1; 
                                    ent.vx += (toCenterX / cDist) * 0.28; ent.vz += (toCenterZ / cDist) * 0.28; 
                                }
                                if (d < 140 && !ent.isCooldown && Math.random() < 0.025) { 
                                    ent.isDashing = true; ent.hasHit = false; ent.hasScoredHit = false; 
                                    ent.dashFrames = 10; ent.dashDirX = (dx / d) * 7; ent.dashDirZ = (dz / d) * 7; 
                                }
                            }
                        }
                        ent.vx *= 0.86; 
                        ...
                        if (dist < minDistSum) {
                            ...
                            if (e2.staggerTimer > 0 || e2.isStunned) { 
                                ...
                                if (e2Speed > 0.1) {
                                    ...
                                    if (angleDeg <= 45) { angleMultiplier = 1.0; } 
                                    else if (angleDeg <= 135) { angleMultiplier = 1.4; } 
                                    else { angleMultiplier = 2.2; }
                                }
                            }
                ...
                entities.forEach((ent, i) => {
                    ...
                    if (dCenter + ent.radius > arenaRadius) {
                        if (gameMode === 'quick') {
                            if (ent.isPlayer) {
                                ent.x = 0; ent.y = 120; ent.vx = 0; ent.vz = 0; ent.survivalTime = 0;
                ...
                if (gameMode === 'private') {
                    ...
                    if (aliveEntities.length <= 1) {
                        if (aliveEntities.length === 1 && !eliminationOrder.includes(aliveEntities[0])) { 
                ...
                if (trail) {
                    ...
                    if (speed > 0.02) {
                        if (trail.points.length === 0) { 
                            ...
                            if (dist > 0.8) { 
                                ...
                                if (trail.points.length > trail.segs + 1) { trail.points.pop(); } 
                            } else { 
                    ...
                    if (hasMoved) {
                        ...
                        if (!trail.mesh.geometry.index) { 
                            let indices = []; 
                            for (let j = 0; j < trail.segs; j++) { 
                                let i0 = j * 2, i1 = j * 2 + 1, i2 = (j + 1) * 2, i3 = (j + 1) * 2 + 1; 
            ...
            if (player) {
                ...
                if (player.isAlive && dashBtn && timerRing && textContent) {
                    ...
                    if (player.isCooldown) { 
                        ...
                        if (player.comboCount > 0) { 
                            ...
                            timerRing.style.strokeDashoffset = 264 * (1 - (player.comboTimer / 300)); 
                        } else { 
                            dashBtn.className = ""; 
                            dashBtn.style.background = baseColor; 
                ...
                if (player.isAlive) {
                    ...
                    if (aliveEntities.length > 0) {
                        ...
                        let st = aliveEntities[spectateTargetIndex];
                        gameCam.position.x += (st.x - gameCam.position.x) * 0.08; 
                        gameCam.position.z += ((st.y + 240) - gameCam.position.z) * 0.08; 
                        gameCam.position.y = 420; 
                        gameCam.lookAt(st.x, 0, st.y);
                    } else { 
...
function showResults() {
    ...
    if (dashBtn) { dashBtn.style.background = ""; dashBtn.style.borderColor = ""; }
